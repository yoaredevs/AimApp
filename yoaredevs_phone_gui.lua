...
-- imagem). O id 418236696 usado no ícone "google" é um decal, por isso
-- nunca aparecia — só o "G" placeholder.
--
-- Em vez de te obrigar a subir a imagem manualmente, o script agora
-- resolve o decal -> imagem real sozinho, em runtime, via game:GetObjects
-- (funciona em qualquer executor com HttpEnabled). Resultado fica em
-- cache pra não repetir a conversão.
--
-- Se quiser forçar uma imagem sua (ex.: você mesmo subiu um PNG em
-- create.roblox.com/dashboard/creations/images), só colar o ID de
-- IMAGEM (não de decal) aqui. Deixando 0, usa a conversão automática.
local GOOGLE_IMAGE_ID = 0
local GOOGLE_DECAL_ID  = 418236696 -- id que já estava no LUCIDE_ASSETS["google"]
...
task.spawn(function()
    ...
    -- algumas vezes antes de desistir e ficar no "G" placeholder.
    for _ = 1, 3 do
        GOOGLE_RESOLVED_IMAGE = decalToImageId(GOOGLE_DECAL_ID)
