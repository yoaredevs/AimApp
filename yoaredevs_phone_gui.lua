...
local GOOGLE_TRY_IDS    = { 431601584, 418236696 } -- ids candidatos, na ordem de tentativa
...
local _googlePlaceholders  = {} -- {image=ImageLabel, placeholder=TextLabel} pra alternar
...
local function lucideIcon(parent, name, color, size)
...
local function addAppIcon(name, label, icon, layoutOrder, color)
    ...
    lucideIcon(iconBox, icon, Color3.fromRGB(255, 255, 255), 30)
...
do
    ...
    local espColor = Color3.fromHSV(espState.hue, 0.75, 1)

    -- botão que abre o popup de cor
    local colorBtn = create("TextButton", {
        Parent = scroll, Size = UDim2.new(1, 0, 0, 44),
        BackgroundColor3 = CONFIG.CardColor, LayoutOrder = 0,
        Text = "", AutoButtonColor = false,
    }, { corner(12) })

    create("TextLabel", {
        Parent = colorBtn, Position = UDim2.new(0, 12, 0, 0), Size = UDim2.new(1, -60, 1, 0),
        BackgroundTransparency = 1, Font = Enum.Font.GothamBold, TextSize = 13,
        TextColor3 = CONFIG.TextColor, TextXAlignment = Enum.TextXAlignment.Left,
        Text = "Cor do ESP",
    }, {})
    ...
    local colorSwatch = create("Frame", {
        Parent = colorBtn, AnchorPoint = Vector2.new(1, 0.5),
        Position = UDim2.new(1, -12, 0.5, 0), Size = UDim2.new(0, 26, 0, 26),
        BackgroundColor3 = espColor, BorderSizePixel = 0, ZIndex = 2,
    }, { corner(8) })

    -- ===== POPUP DE COLOR PICKER =====
    local colorPopup = create("Frame", {
        Parent = PhoneFrame,
        AnchorPoint = Vector2.new(0.5, 0.5),
        Position = UDim2.new(0.5, 0, 0.5, 0),
        Size = UDim2.new(1, -44, 0, 208),
    ...
    create("TextLabel", {
        Parent = colorPopup, Position = UDim2.new(0, 14, 0, 10),
        Size = UDim2.new(1, -50, 0, 20), BackgroundTransparency = 1,
        Font = Enum.Font.GothamBold, TextSize = 14, ZIndex = 91,
        TextColor3 = CONFIG.TextColor, TextXAlignment = Enum.TextXAlignment.Left,
        Text = "Escolha a cor do ESP",
    }, {})

    local closePopup = create("TextButton", {
        Parent = colorPopup, AnchorPoint = Vector2.new(1, 0),
        Position = UDim2.new(1, -8, 0, 8), Size = UDim2.new(0, 26, 0, 26),
        BackgroundColor3 = Color3.fromRGB(45, 45, 52), Text = "",
        AutoButtonColor = false, ZIndex = 91,
    }, { corner(8) })
    do
        local cx = create("Frame", {
            Parent = closePopup, Position = UDim2.new(0, 5, 0, 5),
            Size = UDim2.new(0, 16, 0, 16), BackgroundTransparency = 1, ZIndex = 92,
        }, {})
        lucideIcon(cx, "x", Color3.fromRGB(255, 255, 255), 15)
    end
    ...
    local swatchHolder = create("Frame", {
        Parent = colorPopup, Position = UDim2.new(0, 12, 0, 40),
        Size = UDim2.new(1, -24, 1, -52), BackgroundTransparency = 1, ZIndex = 91,
    }, {})
    ...
    for i, c in ipairs(SWATCHES) do
        local sw = create("TextButton", {
            Parent = swatchHolder, LayoutOrder = i,
            BackgroundColor3 = c, Text = "", AutoButtonColor = false, ZIndex = 92,
        }, { corner(10) })
...
do
    ...
    -- logo grande em cima do "Coming soon". Cria os dois (imagem + "G")
    -- e deixa visível só o certo; _refreshGoogleImage troca sozinho
    -- assim que o decal terminar de resolver (ou já resolvido, se
    -- este app for aberto depois do primeiro load).
    local googleLogoImg = create("ImageLabel", {
        ...
        BackgroundTransparency = 1,
        Image = GOOGLE_RESOLVED_IMAGE or "",
        ScaleType = Enum.ScaleType.Fit,
        Visible = GOOGLE_RESOLVED_IMAGE ~= nil,
    }, {})
    ...
    table.insert(_googlePlaceholders, { image = googleLogoImg, placeholder = googleLogoPlaceholder })
